...
// User represents a system user with detailed information.
type User struct {
	UserID     int64       `json:"userID"` // Unique identifier for the user.
	Email      string      `json:"email"`  // User's email address.
	Password   string      `json:"-"`      // Excluded from JSON to protect sensitive data.
	FirstName  string      `json:"firstName"`
	LastName   string      `json:"lastName"`
	Nickname   string      `json:"nickname"`
	Created    time.Time   `json:"created"` // Account creation timestamp.
	Image      *string     `json:"image"`   // Optional user profile image.
	About      string      `json:"about"`   // User bio or description.
	Birthday   time.Time   `json:"birthday"`
	Private    bool        `json:"private"`    // Whether the profile is private.
	FollowInfo *FollowInfo `json:"followInfo"` // Follow relationship data.
}

// UserIncoming is used for creating or updating user records.
// All fields are pointers to allow distinguishing between "no value" and explicit null.
type UserIncoming struct {
	...
	Birthday  *time.Time `json:"birthday"`
	Private   bool       `json:"private"` // Default to false if omitted.
}
...
// pointerSlice returns pointers to the User fields for database row scanning.
