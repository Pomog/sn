...
var loggerEvent = log.New(os.Stdout, "EventModel: ", log.LstdFlags)

// Event represents an event with metadata and participation details.
type Event struct {
...
func (model EventModel) GetByGroup(groupID, myID int64) ([]*Event, error) {
	...
	defer func(rows *sql.Rows) {
		err := rows.Close()
		if err != nil {
			loggerEvent.Printf("Error in GetByID: %v", err)
		}
	}(rows)
	if err != nil {
